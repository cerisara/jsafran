...
public class SuiteDeMots {
	...
	/**
	 * il faut que la suite soit alignée avec une autre suite !
	 * @return
	 */
	public int getInsError() {
		int err=0;
		for (int i=0;i<getNmots();i++) {
			int[] l = getLinkedWords(i);
			if (l.length==0) err++;
		}
		return err;
	}
	/**
	 * on suppose qu'un item ne peut etre lie qu'a un seul autre item !
	 * @return
	 */
	public int getDelError() {
		...
		for (int i=0;i<getNmots();i++) {
			int[] l = getLinkedWords(i);
			if (l.length!=0) linked.add(l[0]);
		}
		return getLinkedSuite().getNmots()-linked.size();
	}
	public int getSubstError() {
		int err=0;
		for (int i=0;i<getNmots();i++) {
			int[] l = getLinkedWords(i);
			if (l.length>0) {
				if (!getMot(i).equals(getLinkedSuite().getMot(l[0]))) err++;
			}
		}
		return err;
	}
	...
	/**
	 * cherche dans this une sous-suite qui matche au mieux la suite s
	 * 
	 * @param s
	 * @return nb d'erreurs
	 */
	public float lookForSubsuite(SuiteDeMots s) {
		float errmin=Float.MAX_VALUE;
		int bestpos = -1;
		for (int i=0;i<getNmots();i++) {
			for (int l=1;l<s.getNmots()+10;l++) {
				if (i+l>=getNmots()) break;
				String[] shortsuit = new String[l];
				for (int j=0;j<l;j++) shortsuit[j]=getMot(i+j);
				SuiteDeMots ss = new SuiteDeMots(shortsuit);
				s.align(ss);
				float err = ((float)s.getInsError()*0.5f+(float)s.getDelError()*0.5f+s.getSubstError())/(float)l;
				if (err<errmin) {
					errmin=err;
					bestpos=i;
				}
	...
	final String digits[] = {"zéro","un","deux","trois","quatre","cinq","six","sept","huit","neuf"};
	private String normalizeNumbers(String s) {
		/*
	...
	private String dizaines(String s) {
