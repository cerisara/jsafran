 ...
 *
 */
public class CompRules implements Rules, Serializable {
	private static final long serialVersionUID = 1L;

	public interface SingleRule extends Serializable {
		public int[] getApplicable(DetGraph g);
		public int[] applyRule(DetGraph g, int pos);
		public String getName();
	}
	ArrayList<SingleRule> localrules = new ArrayList<CompRules.SingleRule>();

	public CompRules() {
		localrules.add(new GP());
		localrules.add(new twoNAM());
		localrules.add(new suj());
		localrules.add(new gpcomp());
		localrules.add(new obj());
		localrules.add(new atts());
		localrules.add(new rel());
		localrules.add(new aux());
		//		localrules.add(new modA());
		//		localrules.add(new modB());
		localrules.add(new heure());
		localrules.add(new det());

		//		localrules.add(new root());

		// rules for ETAPE corpus
		localrules.add(new euh());
		//		localrules.add(new repet1mot());
		localrules.add(new repet());
		localrules.add(new firstDM());
	}

	class firstDM implements SingleRule {
	...
	class repet implements SingleRule {
	...
	class euh implements SingleRule {
		@Override
		public int[] getApplicable(DetGraph g) {
			ArrayList<Integer> res = new ArrayList<Integer>();
			if (g.getNbMots()>1)
	...
	class repet1mot implements SingleRule {
		@Override
		public int[] getApplicable(DetGraph g) {
			ArrayList<Integer> res = new ArrayList<Integer>();
			if (g.getNbMots()>1)
	...
	// =================================================================
	class GP implements SingleRule {
			...
			for (int i=0;i<g.getNbMots();i++) {
				...
				if (pos.startsWith("PRP")) {
					if (i==0) {
						...
						int j;
						for (j=i-1;j>=0;j--) {
							if (g.getMot(j).getPOS().startsWith("VER")) {
	...
	// =================================================================
	class twoNAM implements SingleRule {
	...
	// =================================================================
	class suj implements SingleRule {
			...
			for (int i=0;i<g.getNbGroups();i++) {
				if (g.getGroupName(i).equals("GNHEAD")) {
					...
					for (int j=gov+1;j<g.getNbMots();j++) {
						...
						if (pos.startsWith("VER") && !pos.endsWith("infi")) {
	...
	// =================================================================
	class gpcomp implements SingleRule {
		...
		public int[] getApplicable(DetGraph g) {
			ArrayList<Integer> pp = new ArrayList<Integer>();
			for (int i=0;i<g.getNbMots()-1;i++) {
				if (g.getMot(i).getPOS().startsWith("PRP")) {
	...
	// =================================================================
	class obj implements SingleRule {
	...
	// =================================================================
	class atts implements SingleRule {
	...
	// =================================================================
	class rel implements SingleRule {
							...
							if (g.getMot(j).getPOS().startsWith("VER")) {
								if (g.getMot(j).getLemme().equals("avoir")||g.getMot(j).getLemme().equals("être")) {
									...
									for (int k=j+1;k<g.getNbMots();k++) {
										if (g.getMot(k).getPOS().startsWith("VER")) {
		...
		public int[] applyRule(DetGraph g, int pronompos) {
			...
			for (verb=pronompos+1;verb<g.getNbMots();verb++) {
				if (g.getMot(verb).getPOS().startsWith("VER")) {
	...
	// =================================================================
	class aux implements SingleRule {
		...
		public int[] getApplicable(DetGraph g) {
			...
			for (int i=0;i<g.getNbMots();i++) {
				...
				if (l.equals("être")||l.equals("avoir")) {
					...
					while (j<g.getNbMots()&&(g.getMot(j).getPOS().startsWith("ADV")||g.getMot(j).getPOS().startsWith("PRO:PER"))) j++;
		...
		public int[] applyRule(DetGraph g, int auxpos) {
			int v=auxpos+1;
			while (g.getMot(v).getPOS().startsWith("ADV")||g.getMot(v).getPOS().startsWith("PRO:PER")) v++;
			if (g.getMot(v).getLemme().equals("être")&&v+1<g.getNbMots()&&g.getMot(v+1).getPOS().equals("VER:pper")) {
	...
	// =================================================================
	class modA implements SingleRule {
	...
	// =================================================================
	class heure implements SingleRule {
	...
	// =================================================================
	class det implements SingleRule {
	...
	// =================================================================
	class modB implements SingleRule {
	...
	// =================================================================
	class root implements SingleRule {
